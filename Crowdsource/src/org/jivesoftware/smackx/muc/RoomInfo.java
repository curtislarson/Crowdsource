...
public class RoomInfo {
    ...
    RoomInfo(DiscoverInfo info) {
        ...
        if (form != null) {
            ...
            try
            {
            	values = form.getField("muc#roominfo_subject").getValues();
                if (values.hasNext()) {
            ...
            //For some reason muc#roominfo_occupants likes to contain the subject line as well, so we do next().next()!
            Iterator<String> occvalues = form.getField("muc#roominfo_occupants").getValues();
            occvalues.next();
            this.occupantsCount =Integer.parseInt(occvalues.next());
        }
    }
